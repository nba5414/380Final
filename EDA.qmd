...
>>>>>>> e0b9a5c2b88c2912d0b874479a9d9faebf5aead2
...
lm_fit2 <- lm(original_price ~ ., data = df2)
...
```{r}
M <- model.matrix(price ~ 0 + . , data = df_train1)

fit_nn1 <- nn_model %>% 
    setup(loss = nn_mse_loss(),
        optimizer = optim_adam, 
        metrics = list(luz_metric_accuracy())) %>%
    set_hparams(p = ncol(M), q1 = 16, q2 = 32, q3 = 16) %>%
...
```{r}
library(glmnet)
x <- data.matrix(df %>% select(!price))
y <- df$price

lasso1 <- cv.glmnet(x, y, alpha = 1)
ridge1 <- cv.glmnet(x, y, alpha = 0)

rmse(y, predict(lasso, x))
rmse(y, predict(ridge, x))
```
...
```{r}
rmse(df$price, lm_fit$fitted.values)
```

```{r}
null_model <- lm(price ~ 1, df)
forward1 <- step(null_model, direction = 'forward', scope = formula(lm_fit))

backward1 <- step(lm_fit, direction = 'backward')

rmse(df$price, predict(forward))
rmse(df$price, predict(backward))
```

```{r}
rmse(df2$original_price, predict(lm_fit2))
```
